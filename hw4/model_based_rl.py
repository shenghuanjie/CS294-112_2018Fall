...
class ModelBasedRL(object):
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            f.savefig(os.path.join(logger.dir, 'prediction_{0:03d}.png'.format(r_num)), bbox_inches='tight')

        logger.info('All plots saved to folder')

    def _debug_plot_states(self, states, pred_states, r_num):
        state_dim = states.shape[1]
        rows = int(np.sqrt(state_dim))
        cols = state_dim // rows
        f, axes = plt.subplots(rows, cols, figsize=(3 * cols, 3 * rows))
    ...
    def _debug_rewards(self, dataset):
        end_idxs = np.nonzero(dataset._dones)[0] + 1
        ...
        for end_idx in end_idxs:
            rewards = dataset._rewards[start_idx:end_idx]
            plt.plot(np.arange(start_idx, end_idx, 1), rewards)
            returns.append(rewards)
            start_idx = end_idx
        plt.subplot(122)
        plt.hist(returns)
        plt.savefig(os.path.join(logger.dir, 'summary.png'))
    ...
    def run_bonus_q2(self):
        ...
        ### YOUR CODE HERE
        # raise NotImplementedError
        eval_dataset = self._gather_rollouts_cross_entropy(self._policy, self._num_onpolicy_rollouts)
    ...
    def run_bonus_q3(self):
        ...
        for itr in range(self._num_onpolicy_iters + 1):
            ...
            logger.info('Gathering rollouts...')
            # raise NotImplementedError
            new_dataset = self._gather_rollouts_cross_entropy(self._policy, self._num_onpolicy_rollouts)
