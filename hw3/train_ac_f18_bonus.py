...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        for _ in range(n_layers):
            nn = tf.layers.dense(nn, size, activation=activation)
...
class Agent(object):
    def __init__(self, actor_graph_args, value_graph_args, sample_trajectory_args, estimate_advantage_args):
        super(Agent, self).__init__()
        self.ob_dim = actor_graph_args['ob_dim']
        self.ac_dim = actor_graph_args['ac_dim']
        self.discrete = actor_graph_args['discrete']
        self.size = actor_graph_args['size']
        self.n_layers = actor_graph_args['n_layers']
        self.learning_rate = actor_graph_args['learning_rate']
        self.num_target_updates = actor_graph_args['num_target_updates']
        self.num_grad_steps_per_target_update = actor_graph_args['num_grad_steps_per_target_update']

        # BONUS
        self.value_n_layers = value_graph_args['value_n_layers']
        self.value_size = value_graph_args['value_size']
        self.value_learning_rate = value_graph_args['value_learning_rate']
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            sy_sampled_ac = sy_mean + z * tf.reshape(tf.exp(sy_logstd), [1, self.ac_dim])
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            sy_ac_na = tf.stop_gradient(tf.one_hot(sy_ac_na, self.ac_dim))
            ...
            mvn = tf.contrib.distributions.MultivariateNormalDiag(
                loc=sy_mean, scale_diag=tf.exp(sy_logstd), name='MND'
            )
            sy_logprob_n = tf.log(mvn.prob(sy_ac_na), name='sy_logprob_n')
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        self.critic_prediction = tf.squeeze(build_mlp(
            ...
            1,
            "nn_critic",
            n_layers=self.value_n_layers,
            size=self.value_size))
        self.sy_target_n = tf.placeholder(shape=[None], name="critic_target", dtype=tf.float32)
        self.critic_loss = tf.losses.mean_squared_error(self.sy_target_n, self.critic_prediction)
        self.critic_update_op = tf.train.AdamOptimizer(self.value_learning_rate).minimize(self.critic_loss)
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            obs.append(ob)
            # raise NotImplementedError
            ac = self.sess.run(self.sy_sampled_ac, feed_dict={self.sy_ob_no: np.atleast_2d(ob)})
            ac = ac[0]
            acs.append(ac)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        # raise NotImplementedError
        v_stp1 = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        v_st = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
        q_st_at = re_n + (self.gamma * v_stp1) * (1 - terminal_n)
        adv_n = q_st_at - v_st

        if self.normalize_advantages:
            # raise NotImplementedError
            adv_n = self.normalize_np_array(adv_n)
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
            ...
            for j in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={self.sy_ob_no: ob_no,
                                                                self.sy_target_n: q_st_at})
    ...
    def normalize_np_array(self, data, mean=0, std=1.0, eps=1e-8):
...
def train_AC(
    ...
    agent = Agent(actor_graph_args, value_graph_args, sample_trajectory_args, estimate_advantage_args)  # estimate_return_args
...
def setup_inputs():
    ...
    # BONUS
    parser.add_argument('--value_n_layers', '-vl', type=int, default=2)
    parser.add_argument('--value_size', '-vs', type=int, default=64)
    parser.add_argument('--value_learning_rate', '-vlr', type=float, default=5e-3)

    return parser


def prepdirs(args, prob=None):
...
def train_func(inputs):
    args = inputs['args']
    seed = inputs['seed']
    max_path_length = inputs['max_path_length']
...
def main():
    ...
    # I don't use command line so everything is parsed here
    args = parser.parse_args()

    # make folders we need
    logdir = prepdirs(args)

    # run actual analysis
    analyze(args, logdir)


def analyze(args, logdir):
    ...
    for e in range(args.n_experiments):
        ...
        # YOUR OWN CODE
        # moved the local function out
        inputs = {'args': args, 'seed': seed, 'max_path_length': max_path_length, 'logdir': logdir}

        # # Awkward hacky process runs, because Tensorflow does not like
        # # repeatedly calling train_PG in the same thread.
        p = Process(target=train_func, args=(inputs, ))
        p.start()
        processes.append(p)
...
def read_data(all_logdirs):
    data = []
    for ilog, logdir in enumerate(all_logdirs):
        data += get_datasets(logdir)
    data = pd.concat(data, ignore_index=True)
    return data


def grid_search(brange, lrange, name='prob'):
    # set up input arguments
    parser = setup_inputs()
    ...
    for ib, ba in enumerate(brange):
        for il, lr in enumerate(lrange):

            # modify batch size and learning rate
            args = modify_args(inputs, batch_size=ba,
                               learning_rate=lr,
                               exp_name='hc_b{0:d}_r{1:.2g}'.format(ba, lr).replace('.', '-'))
            all_ba_lr.append([ba, lr])

            # make folders we need
            logdir = prepdirs(args, prob=name)
    ...
    for ilog, logdir in enumerate(all_logdirs):
        # evaluate the reward and save the max
        df = read_data([logdir])
        score = np.mean(df.loc[df['Iteration'] == 99, 'AverageReturn'])
        if max_score <= score:
            max_score = score
            max_ba = all_ba_lr[ilog][0]
            max_lr = all_ba_lr[ilog][1]

    print('max_score:{}, batch_size:{}, learning_rate:{}'.format(max_score, max_ba, max_lr))
    with open(os.path.join('./data/' + name + '/' + name + '.txt'), 'w') as f:
        all_logdirs = 'python plot.py ' + ' '.join(all_logdirs)
        print(all_logdirs, file=f)
