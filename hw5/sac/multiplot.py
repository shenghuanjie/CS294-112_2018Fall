...
def plot_datasets(datasets, values=("AverageReturn",), expnames=[]):
    """
    There are lots of bugs in this function, but it works for my purposes.
    :param data:
    :param values:
    :return:
    """
    ...
    colors = [u'r', u'b', u'k', u'g', u'c', u'm', u'y']
    linestyles = ["-", "--", "-.", ":"]
    ...
    for id, data in enumerate(datasets):
        for iv, value in enumerate(values):
            legends.append(expnames[id]+'_'+values[iv])
            sns.tsplot(data=data, time="Iteration", value=value, unit="Unit",
                       condition="Condition", color=colors[id % len(colors)],
                       linestyle=linestyles[iv % len(linestyles)])
    plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
    plt.legend(legends, loc='best').draggable()
    plt.show()
...
def get_datasets(fpath, condition=None):
    ...
    return datasets, condition or exp_name
...
def main():
    ...
    if use_legend:
        for logdir, legend_title in zip(args.logdir, args.legend):
            dataset, expname = get_datasets(logdir, legend_title)
            expnames.append(expname)
            data.append(pd.concat(dataset, ignore_index=True))
    else:
        for logdir in args.logdir:
            dataset, expname = get_datasets(logdir)
            expnames.append(expname)
            data.append(pd.concat(dataset, ignore_index=True))

    plot_datasets(data, values=values, expnames=expnames)
